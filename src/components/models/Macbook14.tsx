...
Command: npx gltfjsx@6.5.3 src/assets/models/macbook-14.glb -T 
Files: src/assets/models/macbook-14.glb [629.72KB] > D:\PROJECT HERRY\WEB\webmacbook\macbook-14-transformed.glb [627.9KB] (0%)
...
export default function Macbook14Model(props: any) {
  const { nodes, materials, scene } = useGLTF('/models/macbook-14-transformed.glb') as any
  const { selectedColor } = useMacbookStore()

  const textture = useTexture('/models/screen.png')
    textture.colorSpace = SRGBColorSpace;
    ...
    scene.traverse((child: any) => {
      if (child.isMesh) {
        if(!noChangeParts.includes(child.name)) {
          child.material.color = new Color(selectedColor);
        }
      }
    });
  }, [selectedColor, scene]);
